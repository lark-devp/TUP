...
TaskSelectorWindow::TaskSelectorWindow(QWidget *parent) : QWidget(parent) {
    ...
    taskList->addItem("Задача 1: Сделать отчёт");
    ...
    multiTaskBtn->setStyleSheet(
        "QPushButton {"
        "   background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #d0d0d0, stop:1 #909090);"
        "   color: white;"
        "   border-radius: 12px;"
        "   font-size: 16px;"
        "   font-weight: bold;"
        ...
        "}"
        "QPushButton:pressed {"
        "   background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #909090, stop:1 #707070);"
        "}"
        );
